...
cd /tmp
curl -k  -L -o ${img_file}.gz $img_url
if [ $? -ne 0 ]; then
...
curl -k  -s -L -o /tmp/Storage/syslinux.tar.bz2 $tar_file 
if [ $? -ne 0 ]; then
...
#get/extract syslinux
tar_file="${util_source}${syslinux_version}.tar.bz2"
curl -k  -s -L -o /tmp/Storage/syslinux.tar.bz2 $tar_file > /dev/null 2>&1
if [ $? -ne 0 ]; then
	LE_install_error "Failed to download syslinux; check your Internet connection and try again"
fi
cd /tmp/Storage
